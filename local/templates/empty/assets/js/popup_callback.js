$(document).ready(() => {
    ...
    $('[data-target=callback]').on('click', function (event) {
        event.preventDefault();
        const $cardDesc = $(this).siblings('.card__desc');
        $productName.val($cardDesc.find('.card__name').text());
        $productPrice.val($cardDesc.find('.card__price').text());
        $productSize.val($cardDesc.find('.card__property').text());
        callbackOpen();
    });
